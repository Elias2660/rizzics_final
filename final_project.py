...
BALL_RADIUS_INIT = 1  # radius of the ball (meters), should be BALL_RADIUS_INIT
...
while True:
    ...
    if started:
        ...
        height_submerged = 0
        if yy - ball_radius >= FLUID_Y + FLUID_HEIGHT / 2:
            # the ball is above the water
            # print("above the water")
            height_submerged = 0
        elif yy + ball_radius <= FLUID_Y + FLUID_HEIGHT / 2:
            # print("fully submerged")
            # the ball is fully submerged
            height_submerged = 2 * ball_radius
        else:
            # the ball is somewhat submerged
            # print("partially submerged")
            height_submerged = (FLUID_Y + FLUID_HEIGHT / 2) - (
                yy - ball_radius
            )
